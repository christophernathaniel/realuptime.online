...
import { Activity, Copy, KeyRound, Mail, Save, ServerCog, Trash2, TriangleAlert, type LucideIcon } from 'lucide-react';
...
type IntegrationSection = 'webhooks' | 'api' | 'contacts' | 'operations';
...
const workflowPayloadExample = JSON.stringify({
    ...
    monitor_url: 'https://app.example.com/monitors/01JQ5S8W3F7M6T2N9K4R1C8HZX',
    ...
    return (
        <button
            ...
            className={cn(
                ...
                active
                    ? 'border-[#7c8cff]/40 bg-[#171c33] shadow-[0_0_0_1px_rgba(124,140,255,0.14)_inset]'
                    : 'border-[#2b3544] bg-[#121821] hover:border-[#3b4656] hover:bg-[#161d29]',
            )}
            aria-pressed={active}
        >
            <div className="flex items-start justify-between gap-4">
                <div>
                    <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#7f8b9b]">
                        {label}
                    </div>
                    <div className="mt-2 text-[28px] font-semibold tracking-[-0.05em] text-white">{value}</div>
                    <div className="mt-1 text-sm text-[#dce6fb]">{detail}</div>
                </div>
                <span
                    className={cn(
                        'inline-flex size-11 items-center justify-center rounded-[16px] border',
                        active ? 'border-[#7c8cff]/30 bg-[#7c8cff]/12 text-[#dce1ff]' : 'border-[#2b3544] bg-[#171d28] text-[#9ca7b9]',
                    )}
                >
                    <Icon className="size-5" />
                </span>
            </div>
            <div className="mt-4 text-[14px] leading-6 text-[#9ca7b9]">{description}</div>
        </button>
    );
}
...
export default function IntegrationsPage({
    ...
    const [activeSection, setActiveSection] = useState<IntegrationSection>('webhooks');
    ...
    const sectionCards: Array<{
    ...
    }> = [
        {
            id: 'webhooks',
            label: 'Webhooks',
            value: `${summary.activeIntegrations}/${summary.integrations}`,
            detail: 'active destinations',
            description: 'Workflow-ready outbound payloads for Slack, Zapier, Make, monday.com, or your own endpoints.',
            icon: ServerCog,
            title: 'Webhook automations',
            intro: 'Create outbound workflow integrations, test them, and inspect the exact flat payload fields your automation tool receives.',
        },
        {
            id: 'api',
            label: 'API tokens',
            value: summary.apiTokens,
            detail: 'issued workspace tokens',
            description: 'Bearer tokens for scripts, CI jobs, internal dashboards, and external automation.',
            icon: KeyRound,
            title: 'REST API access',
            intro: 'Create and rotate bearer tokens for the RealUptime API, with quick-start examples for scripts and external services.',
        },
        {
            id: 'contacts',
            label: 'Email contacts',
            value: `${summary.enabled}/${summary.contacts}`,
            detail: 'enabled recipients',
            description: 'Email destinations for alerts, operator routing, and primary escalation coverage.',
            icon: Mail,
            title: 'Email notification routing',
            intro: 'Manage the people and inboxes that receive alert emails, including defaults and primary contact behavior.',
        },
        {
            id: 'operations',
            label: 'Delivery ops',
            value: summary.emailsFailed,
            detail: summary.emailsPending > 0 ? `${summary.emailsPending} pending deliveries` : `${summary.emailsSent} sent notifications`,
            description: 'Queue health, runtime configuration, and recent notification delivery outcomes.',
            icon: Activity,
            title: 'Runtime and delivery operations',
            intro: 'Inspect the queues and runtime settings behind your outgoing notifications, and review recent delivery results in one place.',
        },
    ];

    const currentSection = sectionCards.find((section) => section.id === activeSection) ?? sectionCards[0];
        ...
        <MonitoringLayout>
            ...
            <div className="space-y-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <div className="mt-2 max-w-[820px] text-[16px] text-[#9ca7b9]">
                            Manage workflow webhooks, REST API credentials, email contacts, and delivery operations from one grouped surface.
                        </div>
                    </div>
                </div>
                ...
                <PageCard className="p-6">
                    ...
                    <div className="mt-2 text-[24px] font-semibold tracking-[-0.05em] text-white">{currentSection.title}</div>
                    <div className="mt-2 max-w-[840px] text-[15px] text-[#9ca7b9]">{currentSection.intro}</div>
                </PageCard>
                        ...
                        <aside className="space-y-4">
                            <PageCard className="space-y-4 p-6">
                                ...
                                <div className="text-[14px] text-[#9ca7b9]">
                                    Use these exact fields when mapping variables inside Slack workflows or other automation tools.
                                </div>
                            ...
                            <PageCard className="space-y-4 p-6">
                                <div className="text-[18px] font-semibold text-white">Testing workflow setup</div>
                                <div className="space-y-3 text-[14px] leading-6 text-[#9ca7b9]">
                                    <div>Save the integration first, then use the per-integration <span className="text-white">Test webhook</span> button to send a live payload.</div>
                                    <div>Workflow tools that expect flat variables should map fields like <span className="font-mono text-[#dce6fb]">message_title</span>, <span className="font-mono text-[#dce6fb]">message_summary</span>, <span className="font-mono text-[#dce6fb]">monitor_name</span>, and <span className="font-mono text-[#dce6fb]">monitor_url</span>.</div>
                                    <div>Use the delivery operations tab if you need to inspect queue health or recent notification failures.</div>
                                </div>
                            </PageCard>
                        </aside>
                    </div>
                ...
                {activeSection === 'api' ? (
                    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_420px]">
                        ...
                        <aside className="space-y-4">
                            <PageCard className="space-y-4 p-6">
                                <div className="text-[18px] font-semibold text-white">API quick start</div>
                                <div className="space-y-3 text-[14px] leading-6 text-[#9ca7b9]">
                                    <div>Base URL: <span className="font-mono text-[#dce6fb]">{runtime.apiBaseUrl}</span></div>
                                    <div>Use a bearer token in the <span className="font-mono text-[#dce6fb]">Authorization</span> header for API requests.</div>
                                </div>
                            ...
                            <PageCard className="space-y-4 p-6">
                                <div className="text-[18px] font-semibold text-white">Token handling</div>
                                <div className="space-y-3 text-[14px] leading-6 text-[#9ca7b9]">
                                    <div>Store tokens in a secret manager or CI variable, not in source control.</div>
                                    <div>Revoke old tokens when rotating external automation or dashboard access.</div>
                                    <div>New tokens are shown once, so copy them immediately after creation.</div>
                                </div>
                            </PageCard>
                        </aside>
                    </div>
                ...
                {activeSection === 'contacts' ? (
                    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_420px]">
                        ...
                        <aside className="space-y-4">
                            <PageCard className="space-y-4 p-6">
                                <div className="text-[18px] font-semibold text-white">Contact routing notes</div>
                                <div className="space-y-3 text-[14px] leading-6 text-[#9ca7b9]">
                                    <div>Enabled contacts can receive alert notifications as long as they remain assigned to relevant monitors.</div>
                                    <div>The primary contact is your default recipient for broader workspace coverage and manual operator workflows.</div>
                                    <div>Recent per-contact delivery history appears in the delivery operations tab.</div>
                                </div>
                            </PageCard>

                            <PageCard className="space-y-4 p-6">
                                <div className="text-[18px] font-semibold text-white">Mail runtime</div>
                                <div className="space-y-3">
                                    <RuntimeRow label="Mail driver" value={runtime.mailer} />
                                    <RuntimeRow label="Enabled contacts" value={summary.enabled} />
                                    <RuntimeRow label="Total contacts" value={summary.contacts} />
                                </div>
                            </PageCard>
                        </aside>
                    </div>
