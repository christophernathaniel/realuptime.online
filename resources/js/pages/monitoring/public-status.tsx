...
const tones = {
    up: {
        badge: 'bg-[#3ee072]/15 text-[#3ee072] border-[#3ee072]/25',
        dot: 'bg-[#3ee072]',
    },
    down: {
        badge: 'bg-[#ff6269]/15 text-[#ff9ca1] border-[#ff6269]/25',
        dot: 'bg-[#ff6269]',
    },
    warning: {
        badge: 'bg-[#ffb454]/15 text-[#ffcd84] border-[#ffb454]/25',
        dot: 'bg-[#ffb454]',
    },
    maintenance: {
        badge: 'bg-[#4d7cff]/15 text-[#9bb4ff] border-[#4d7cff]/25',
        dot: 'bg-[#4d7cff]',
    },
};
...
export default function PublicStatusPage({ statusPage }: PublicStatusPageProps) {
    ...
    return (
        <div className="min-h-screen bg-[#081428] px-4 py-8 text-[#f4f7ff] sm:px-6 lg:px-8">
            <Head title={statusPage.headline} />
            <div className="mx-auto max-w-[1180px] space-y-6">
                <div className="rounded-[28px] border border-white/6 bg-[#101a2f] px-7 py-8 shadow-[0_32px_90px_rgba(0,0,0,0.28)]">
                ...
                <section className="rounded-[24px] border border-white/6 bg-[#101a2f] p-6 shadow-[0_24px_60px_rgba(0,0,0,0.2)]">
                    <div className="text-[22px] font-semibold text-white">Monitored services</div>
                    <div className="mt-5 grid gap-4 md:grid-cols-2">
                        {statusPage.monitors.map((monitor) => (
                            <div key={monitor.name} className="rounded-[18px] bg-[#111a2c] px-5 py-5">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <div
                                        className={cn(
                                            ...
                                            monitor.statusTone === 'up' && 'bg-[#3ee072]/15 text-[#3ee072]',
                                            monitor.statusTone === 'down' && 'bg-[#ff6269]/15 text-[#ff9ca1]',
                                            monitor.statusTone === 'warning' && 'bg-[#ffb454]/15 text-[#ffcd84]',
                                            monitor.statusTone === 'maintenance' && 'bg-[#4d7cff]/15 text-[#9bb4ff]',
                                        )}
                                    >
                                        <span
                                            className={cn(
                                                'inline-flex size-2 rounded-full',
                                                monitor.statusTone === 'up' && 'bg-[#3ee072]',
                                                monitor.statusTone === 'down' && 'bg-[#ff6269]',
                                                monitor.statusTone === 'warning' && 'bg-[#ffb454]',
                                                monitor.statusTone === 'maintenance' && 'bg-[#4d7cff]',
                                            )}
                                        />
                                        {monitor.status}
                                    </div>
                                ...
                                {monitor.activeMaintenance ? (
                                    <div className="mt-4 rounded-[14px] bg-[#4d7cff]/12 px-4 py-3 text-[13px] text-[#b6c7ff]">This monitor currently has an active maintenance window.</div>
                                ) : null}
                            </div>
                        ))}
                    </div>
                ...
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
                        ...
                        <div className="mt-5 space-y-4">
                            {statusPage.incidents.length === 0 ? (
                                ...
                                statusPage.incidents.map((incident) => (
                                    <div key={`${incident.title}-${incident.startedAt}`} className="rounded-[18px] bg-[#111a2c] px-5 py-5">
                                        <div className="flex flex-wrap items-center justify-between gap-3 text-[15px] text-white">
                                            <span>{incident.title}</span>
                                            <span className={incident.status === 'Resolved' ? 'text-[#3ee072]' : 'text-[#ffb454]'}>{incident.status}</span>
                                        </div>
                                        <div className="mt-2 flex flex-wrap gap-2 text-[12px] text-[#9bb4ff]">
                                            <span className="rounded-full bg-[#0d1628] px-3 py-1">{incident.impact}</span>
                                            {incident.monitors.map((monitor) => (
                                                <span key={monitor} className="rounded-full bg-[#0d1628] px-3 py-1">{monitor}</span>
                                            ))}
                                        </div>
                                        ...
                                        <div className="mt-4 space-y-3">
                                            {incident.updates.map((update, index) => (
                                                <div key={`${update.createdAt}-${index}`} className="rounded-[14px] bg-[#0d1628] px-4 py-4">
                    ...
                    <aside className="space-y-6">
                        <div className="rounded-[24px] border border-white/6 bg-[#101a2f] p-6 shadow-[0_24px_60px_rgba(0,0,0,0.2)]">
                            ...
                            <div className="mt-5 space-y-4">
                                {statusPage.recentUpdates.length === 0 ? (
                                    ...
                                    statusPage.recentUpdates.map((update, index) => (
                                        <div key={`${update.incidentTitle}-${index}`} className="rounded-[18px] bg-[#111a2c] px-5 py-5">
                                            <div className="flex items-center justify-between gap-3 text-[15px] text-white">
                                                <span>{update.incidentTitle}</span>
                                                <span className="text-[#7f8eab]">{update.createdAt}</span>
                                            </div>
                                            <div className="mt-2 text-[13px] text-[#9bb4ff]">{update.status} • {update.impact}</div>
                                            <div className="mt-2 text-[14px] text-[#dce6fb]">{update.message}</div>
                                        </div>
                                    ))
                                )}
                            ...
                            <div className="mt-5 space-y-4">
                                {statusPage.monitorIncidents.length === 0 ? (
                                    ...
                                    statusPage.monitorIncidents.map((incident) => (
                                        <div key={`${incident.monitor}-${incident.startedAt}`} className="rounded-[18px] bg-[#111a2c] px-5 py-5">
                                            <div className="flex items-center justify-between gap-3 text-[15px] text-white">
