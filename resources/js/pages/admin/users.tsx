    ...
    return (
        <MonitoringLayout>
            ...
            <div className="space-y-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <div className="mt-2 max-w-[860px] text-[16px] text-[#9ca7b9]">
                            Review every signed-up account, inspect plan state
                            and live usage, and open a full support view for
                            billing, monitors, sessions, contacts, and recent
                            activity.
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 xl:grid-cols-5 [&>*:last-child]:col-span-2 xl:[&>*:last-child]:col-span-1">
                        ...
                        <SummaryCard
                            label="Monitors"
                            value={summary.monitors}
                        />
                        <SummaryCard
                            label="Courtesy extensions"
                            value={summary.supportExtensions}
                        />
                        <SummaryCard
                ...
                <PageCard className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex min-w-0 items-start gap-3">
                        {billingConfiguration.ready ? (
                            <CheckCircle2 className="mt-0.5 size-5 shrink-0 text-[#57c7c2]" />
                        ) : (
                            <XCircle className="mt-0.5 size-5 shrink-0 text-[#ff858b]" />
                        )}
                        <div className="min-w-0">
                            ...
                            <div className="mt-1 text-sm text-[#9ca7b9]">
                                {billingConfiguration.ready
                                    ? 'Keys, webhook verification, and both paid price IDs are configured.'
                                    : `Missing: ${billingConfiguration.missing.join(', ')}`}
                            </div>
                        </div>
                    </div>
                    <div className="font-mono text-xs break-all text-[#9badca]">
                        {billingConfiguration.webhookUrl}
                    </div>
                </PageCard>
                            ...
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                                <div>
                                    ...
                                    <div className="mt-2 text-[15px] text-[#9ca7b9]">
                                        Search by name or email, then open the
                                        account for the full support view.
                                    </div>
                                </div>
                                ...
                                >
                                    <label className="relative block">
                                        <Search className="pointer-events-none absolute top-1/2 left-4 size-4 -translate-y-1/2 text-[#7f8b9b]" />
                                        <input
                                            value={search}
                                            onChange={(event) =>
                                            ...
                                            className="h-12 w-full rounded-[16px] border border-white/10 bg-[#0b1425] pr-4 pl-11 text-sm text-white outline-none placeholder:text-[#7f8b9b]"
                                        />
                                    </label>
                                </form>
                            </div>
                            ...
                            <div className="space-y-4">
                                {users.data.length === 0 ? (
                                    ...
                                    users.data.map((user) => (
                                        <div
                                            key={user.id}
                                            className="rounded-[20px] border border-white/8 bg-[#171d28] px-5 py-5"
                                        >
                                            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                                                <div className="min-w-0">
                                                    <div className="flex flex-wrap items-center gap-2 text-[18px] font-semibold text-white">
                                                        <span>{user.name}</span>
                                                        <span className="rounded-full border border-white/8 px-2 py-0.5 text-[11px] tracking-[0.18em] text-[#9ca7b9] uppercase">
                                                            {user.emailVerified
                                                                ? 'Verified'
                                                                : 'Unverified'}
                                                        </span>
                                                        <span className="rounded-full border border-[#4d7cff]/20 bg-[#102240] px-2 py-0.5 text-[11px] tracking-[0.18em] text-[#dce6fb] uppercase">
                                                            {
                                                                user.membershipPlanLabel
                                                            }
                                                        </span>
                                                        {user.isAdmin ? (
                                                            <span className="rounded-full border border-[#7c8cff]/20 bg-[#171c33] px-2 py-0.5 text-[11px] tracking-[0.18em] text-[#dbe1ff] uppercase">
                                                                Main admin
                                                            </span>
                                                        ) : null}
                                                        {user.supportExtension ? (
                                                            <span className="rounded-full border border-[#57c7c2]/20 bg-[#0f2527] px-2 py-0.5 text-[11px] tracking-[0.18em] text-[#9de5e0] uppercase">
                                                                Courtesy until{' '}
                                                                {user
                                                                    .supportExtension
                                                                    .expiresAt ??
                                                        ...
                                                        {auth.user.id ===
                                                        user.id ? (
                                                            <span className="rounded-full border border-white/8 px-2 py-0.5 text-[11px] tracking-[0.18em] text-[#dce6fb] uppercase">
                                                                You
                                                            </span>
                                                        ) : null}
                                                    </div>
                                                    <div className="mt-1 text-sm break-all text-[#9ca7b9]">
                                                        {user.email}
                                                    </div>
                                                    ...
                                                    user.id ? (
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                if (
                                                                    window.confirm(
                                                                        `Delete user "${user.email}"? This also deletes their workspace data.`,
                                                                    )
                                                                ) {
                                                                    router.delete(
                                                                        `/admin/users/${user.id}`,
                                            ...
                                            <div className="mt-3 flex flex-wrap gap-3 text-[13px] text-[#9badca]">
                                                <span className="rounded-full bg-[#121821] px-3 py-2">
                                                    Contacts{' '}
                                                    {user.contactsCount}
                                                </span>
                                                <span className="rounded-full bg-[#121821] px-3 py-2">
                                                    Team members{' '}
                                                    {user.acceptedMembersCount}
                                                </span>
                                                <span className="rounded-full bg-[#121821] px-3 py-2">
                                                    Pending invites{' '}
                                                    {
                    ...
                    <aside className="space-y-4">
                        <PageCard className="space-y-5 p-6">
                            ...
                            <div className="text-[15px] text-[#9ca7b9]">
                                Accounts created here are standard users by
                                default. Use the account view to handle plan
                                support, billing context, and deeper operational
                                troubleshooting.
                            </div>
